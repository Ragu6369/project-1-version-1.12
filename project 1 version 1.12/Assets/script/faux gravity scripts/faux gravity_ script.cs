...
public class dummyScrip_mario : MonoBehaviour
{
    ...
    void ApplyGravity()
    {
        // Case 1: No Planet uses normal downward gravity
        ...
        // Case 2: Planet present sweep rays like reference  
    ...
    {
        // Quaternion dtargetRotation = Quaternion.FromToRotation(transform.up, default_rotation) * transform.rotation;
        if (currentPlanet == null)
        {
            Quaternion dtargetRotation = Quaternion.LookRotation(default_rotation);
