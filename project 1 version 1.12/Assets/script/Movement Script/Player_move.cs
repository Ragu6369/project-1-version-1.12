...
public class Player_move: MonoBehaviour
{
    ...
    [SerializeField] public float default_gravity = -0.05f;
    ...
    {   //declaration:
        ...
        playerInput.player_movement.SPRINT.started += OnSprint;
        playerInput.player_movement.JUMP.started += OnJump;
    ...
    {  
        ...
        {
            char_controller.Move(runmovement *  (speed * 1.2f) * Time.deltaTime);
        }
        else
        {
    ...
    {
        if (context.started && char_controller.isGrounded)
        {
           gravity_y = _jumpForce;
        }
    }

    //---------------------SPRINT INPUT FUNCTION-----------------------
    void OnSprint(InputAction.CallbackContext context)
    {
        if (context.started && IsRunPressed)
        {
    ...
    { 
        ...
        IsSprint = animator.GetBool(IsSprintHash);
        //value updating
        ...
        //FOR SPRINT ANIMATION
        if(IsSprintPressed && !IsSprint)
        {
            animator.SetBool(IsSprintHash, true);
        }
        else if(!IsSprintPressed && IsSprint)
        {
    ...
    void gravity_control()
    {
        ...
        {
            gravity_y += -9.81f * Time.deltaTime;
