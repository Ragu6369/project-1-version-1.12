...
{
    public float parent;
    DummyScrip_mario playerController;

    private void Awake()
    {
        playerController = FindObjectOfType<DummyScrip_mario>();
    }
    /*
    ...
    */
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            if(playerController.currentPlanet == null)
            {
                SetParentWithoutScaleChange(collision.transform, transform.transform);
                playerController.PlatformParented = true;
                playerController.previousParent = null;
                playerController.OnceParented++;
                parent++;
            }
    ...
    private void OnCollisionExit(Collision collision)
    {
        if(collision.gameObject.CompareTag("Player"))
        {
            ...
            playerController.PlatformParented = false;
            parent = 0;
        }
    }
            ...
            foreach(Transform child in transform)
            {
                if(child.CompareTag("Player"))
                {
                    ...
                    if(playerController != null && playerRb != null)
                    {
                        ...
                        playerRb.AddForce(jumpDir * 1000 , ForceMode.Impulse);
        ...
        if (other.CompareTag("Planet"))
        {
            ...
            foreach (Transform child in transform)
            {
                if (child.CompareTag("Player"))
                {
                    ...
                    {
                        //Set currentPlanet  as to null by setting previousPlanet to null
                        playerController.previousParent = null;
